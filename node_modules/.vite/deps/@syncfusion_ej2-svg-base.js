import { G as animationMode, Mt as Browser, Q as Complex, U as Animation, V as ChildProperty, Yt as extend, _ as Component, _t as createElement, l as updateBlazorTemplate, nn as isBlazor, nt as NotifyPropertyChanges, r as compile, rn as isNullOrUndefined, rt as Property, s as resetBlazorTemplate, sn as merge, tt as Event, wt as remove } from "./ej2-base-BWKTOeLN.js";
//#region node_modules/@syncfusion/ej2-svg-base/src/svg-render/svg-renderer.js
/**
* To import utils
*/
var SvgRenderer = function() {
	...
	*/
	SvgRenderer.prototype.createSvg = function(options) {
		if (isNullOrUndefined(options.id)) options.id = this.rootId + "_svg";
		this.svgObj = document.getElementById(options.id);
		if (isNullOrUndefined(document.getElementById(options.id))) this.svgObj = document.createElementNS(this.svgLink, "svg");
	...
	SvgRenderer.prototype.setSVGSize = function(width, height) {
		...
		if (isNullOrUndefined(this.width) || this.width <= 0) this.svgObj.setAttribute("width", width ? width.toString() : !isNullOrUndefined(size) ? size.width.toString() : "0");
		else this.svgObj.setAttribute("width", this.width.toString());
		if (isNullOrUndefined(this.height) || this.height <= 0) this.svgObj.setAttribute("height", height ? height.toString() : "450");
		else this.svgObj.setAttribute("height", this.height.toString());
	};
	/**
	* To draw a path
	*
	...
	*/
	SvgRenderer.prototype.drawPath = function(options) {
		...
		if (path === null) path = document.createElementNS(this.svgLink, "path");
	...
	*/
	SvgRenderer.prototype.drawLine = function(options) {
		...
		if (line === null) line = document.createElementNS(this.svgLink, "line");
	...
	*/
	SvgRenderer.prototype.drawRectangle = function(options) {
		...
		if (rectangle === null) rectangle = document.createElementNS(this.svgLink, "rect");
	...
	*/
	SvgRenderer.prototype.drawCircle = function(options) {
		...
		if (circle === null) circle = document.createElementNS(this.svgLink, "circle");
	...
	*/
	SvgRenderer.prototype.drawPolyline = function(options) {
		...
		if (polyline === null) polyline = document.createElementNS(this.svgLink, "polyline");
	...
	*/
	SvgRenderer.prototype.drawEllipse = function(options) {
		...
		if (ellipse === null) ellipse = document.createElementNS(this.svgLink, "ellipse");
	...
	*/
	SvgRenderer.prototype.drawPolygon = function(options) {
		...
		if (polygon === null) polygon = document.createElementNS(this.svgLink, "polygon");
	...
	*/
	SvgRenderer.prototype.drawImage = function(options) {
		...
		if (!isNullOrUndefined(this.getOptionValue(options, "clip-path"))) img.setAttributeNS(null, "clip-path", this.getOptionValue(options, "clip-path"));
		if (!isNullOrUndefined(options.preserveAspectRatio)) img.setAttributeNS(null, "preserveAspectRatio", options.preserveAspectRatio);
		return img;
	};
	/**
	* To draw a text
	...
	*/
	SvgRenderer.prototype.createText = function(options, label) {
		...
		if (!isNullOrUndefined(label)) text.textContent = label;
		return text;
	};
	/**
	* To create a tSpan
	...
	*/
	SvgRenderer.prototype.createTSpan = function(options, label) {
		...
		if (!isNullOrUndefined(label)) tSpan.textContent = label;
		return tSpan;
	};
	/**
	* To create a title
	...
	*/
	SvgRenderer.prototype.createRadialGradient = function(colors, name, options) {
		...
		} else colorName = colors[0].color.toString();
		return colorName;
	};
	/**
	...
	*/
	SvgRenderer.prototype.drawGradient = function(gradientType, options, colors) {
		...
		for (var i = 0; i < colors.length; i++) {
			...
			if (!isNullOrUndefined(colors[i].style)) stop_1.style.cssText = colors[i].style;
	...
	*/
	SvgRenderer.prototype.setElementAttributes = function(options, element) {
		...
		for (var i = 0; i < keys.length; i++) if (keys[i] === "style") element.style.cssText = options[keys[i]];
		else element.setAttribute(keys[i], options[keys[i]]);
		return element;
	};
	/**
...
//#endregion
//#region node_modules/@syncfusion/ej2-svg-base/src/svg-render/canvas-renderer.js
/**
* To import utils.
*/
/**
...
var CanvasRenderer = function() {
	...
	*/
	CanvasRenderer.prototype.setCanvasSize = function(width, height) {
		var element = document.getElementById(this.rootId);
		var size = !isNullOrUndefined(element) ? element.getBoundingClientRect() : null;
		if (isNullOrUndefined(this.width)) this.canvasObj.setAttribute("width", width ? width.toString() : size.width.toString());
		else this.canvasObj.setAttribute("width", this.width.toString());
		if (isNullOrUndefined(this.height)) this.canvasObj.setAttribute("height", height ? height.toString() : "450");
		else this.canvasObj.setAttribute("height", this.height.toString());
	};
	...
	*/
	CanvasRenderer.prototype.drawRectangle = function(options, canvasTranslate) {
		...
		if (cornerRadius !== null && cornerRadius >= 0) this.drawCornerRadius(options);
		else {
	...
	*/
	CanvasRenderer.prototype.drawPath = function(options, canvasTranslate) {
		...
		for (var i = 0; i < dataSplit.length; i = i + 3) {
			...
			switch (dataSplit[i]) {
				case "M":
					if (!options.innerR && !options.cx) this.ctx.moveTo(x1, y1);
					break;
				case "L":
					if (!options.innerR) this.ctx.lineTo(x1, y1);
					break;
				case "Q":
				...
				case "C":
					...
					break;
				case "A":
					if (!options.innerR) if (options.cx) this.ctx.arc(options.cx, options.cy, options.radius, 0, 2 * Math.PI, options.counterClockWise);
					else {
	...
	*/
	CanvasRenderer.prototype.createText = function(options, label, transX, transY, dy, isTSpan) {
		...
		if (!isNullOrUndefined(fontWeight) && fontWeight.toLowerCase() === "regular") fontWeight = "normal";
		var fontSize = this.getOptionValue(options, "font-size");
		var fontFamily = this.getOptionValue(options, "font-family");
		var font = this.getOptionValue(options, "font-style").toLowerCase() + " " + fontWeight + " " + fontSize + " " + fontFamily;
		...
		if (!isTSpan) {
			this.ctx.translate(options.x + 0 / 2 + (transX ? transX : 0), options.y + (transY ? transY : 0));
	...
	*/
	CanvasRenderer.prototype.createLinearGradient = function(colors) {
		var myGradient;
		if (!isNullOrUndefined(colors[0].colorStop)) myGradient = this.ctx.createLinearGradient(0, 0, 0, this.canvasObj.height);
	...
	*/
	CanvasRenderer.prototype.createRadialGradient = function(colors) {
		var myGradient;
		if (!isNullOrUndefined(colors[0].colorStop)) myGradient = this.ctx.createRadialGradient(0, 0, 0, 0, 0, this.canvasObj.height);
		return this.setGradientValues(colors, myGradient);
	};
	...
	*/
	CanvasRenderer.prototype.setElementAttributes = function(options, element) {
		...
		for (var i = 0; i < keys.length; i++) element.setAttribute(keys[i], values[i]);
		return null;
	};
	/**
...
//#endregion
//#region node_modules/@syncfusion/ej2-svg-base/src/tooltip/interface.js
/** @private */
...
//#endregion
//#region node_modules/@syncfusion/ej2-svg-base/src/tooltip/helper.js
...
function measureText(text, font, themeFontStyle, isHeader) {
	...
	if (typeof text === "string" && (text.indexOf("<") > -1 || text.indexOf(">") > -1)) {
		...
		for (var i = 0; i < textArray.length; i++) if (textArray[i].indexOf("<br/>") === -1) textArray[i] = textArray[i].replace(/[<>]/g, "&");
		text = textArray.join(" ");
	}
...
function findDirection(rX, rY, rect, arrowLocation, arrowPadding, top, bottom, left, tipX, tipY, controlName) {
	...
	if (top) {
		...
		if (arrowPadding !== 0) if (controlName === "RangeNavigator") if (arrowLocation.x - arrowPadding > width / 2) {
		...
		if (arrowLocation.x - arrowPadding > startX) direction = direction.concat(" L " + (startX + rX) + " " + height + " Q " + startX + " " + height + " " + startX + " " + (height - rY) + " z");
		else if (arrowPadding === 0) direction = direction.concat(" L " + (startX + rX) + " " + height + " Q " + startX + " " + height + " " + startX + " " + (height - rY) + " z");
		else direction = direction.concat(" L " + startX + " " + (height + rY) + " z");
	} else if (bottom) {
	...
	switch (shape) {
		...
		case "Pentagon":
			...
			for (var i = 0; i <= 5; i++) {
				...
				if (i === 0) path = "M " + (locX + xValue) + " " + (locY + yValue) + " ";
				else path = path.concat("L " + (locX + xValue) + " " + (locY + yValue) + " ");
			}
			path = path.concat("Z");
			merge(options, { "d": path });
			break;
...
function textElement(options, font, color, parent, themeStyle) {
	...
	if (parent) parent.appendChild(htmlObject);
	return htmlObject;
}
...
//#endregion
//#region node_modules/@syncfusion/ej2-svg-base/src/tooltip/tooltip.js
...
var Tooltip = function(_super) {
	...
	*/
	Tooltip.prototype.preRender = function() {
		...
		if (!this.isCanvas) this.removeSVG();
	...
	Tooltip.prototype.removeSVG = function() {
		...
		if (this.blazorTemplate) resetBlazorTemplate(this.element.id + "parent_template_blazorTemplate");
		if (svgObject && svgObject.parentNode) remove(svgObject);
		if (templateObject && templateObject.parentNode) remove(templateObject);
	};
	/**
	...
	*/
	Tooltip.prototype.render = function() {
		...
		if (!this.template) if (this.split) this.renderSplitTooltip();
		else {
		...
		else if (this.split) for (var splitIndex = 0; splitIndex < this.data.length; splitIndex++) {
	...
	*/
	Tooltip.prototype.renderSplitTooltip = function() {
		...
		if (svgRoot) {
			if (currentCount > prevCount) for (var index = prevCount; index < currentCount; index++) {
				...
				if (!tooltipGroup) {
					...
					var tooltipPath = this.renderer.drawPath({
						...
						"opacity": (this.theme === "TailwindDark" || this.theme === "Tailwind" || this.theme === "Tailwind3Dark" || this.theme === "Tailwind3" || this.theme === "Bootstrap5" || this.theme === "Bootstrap5Dark" || this.theme.indexOf("Fluent2") > -1) && this.opacity === .75 ? 1 : this.opacity,
			...
			else if (currentCount < prevCount) for (var index = currentCount; index < prevCount; index++) {
		...
		for (var splitIndex = 0; splitIndex < currentCount; splitIndex++) this.renderText(this.isFirst, splitIndex);
		if (this.split) this.calculateAutoPositions();
	...
	*/
	Tooltip.prototype.getActualSplitClipRect = function() {
		if (!this.splitClipBounds || this.splitClipBounds.length === 0) return new Rect(0, 0, 0, 0);
		var minY = Infinity;
		var maxY = -Infinity;
		var minX = Infinity;
		var maxX = -Infinity;
	...
	*/
	Tooltip.prototype.calculateAutoPositions = function() {
		if (!this.split || this.splitTooltipRectCollection.length < 1) return;
		...
		if (!this.inverted) {
			for (var _i = 0, _a = this.splitTooltipRectCollection; _i < _a.length; _i++) {
				...
				if (tooltipRect.x + tooltipRect.width > areaBoundsRight) this.hasHorizontalOverflow = true;
			...
			for (var _b = 0, _c = this.splitTooltipRectCollection; _b < _c.length; _b++) {
				...
				tooltipRect.x = this.hasHorizontalOverflow ? averageXPosition - (tooltipRect.width + 2 * TOOLTIP_PADDING) : averageXPosition;
			}
		} else {
			var horizontalItems = this.splitTooltipRectCollection.map(function(rect, originalIndex) {
				return {
					xPosition: rect.x,
					yPosition: rect.y,
					width: rect.width,
				...
				return leftItem.xPosition - rightItem.xPosition;
			});
		...
		var sortedTooltipItems = this.splitTooltipRectCollection.map(function(rect, originalIndex) {
			return {
				xPosition: rect.x,
				width: rect.width,
				yPosition: rect.y,
		...
		while (clusterStartIndex < sortedTooltipItems.length) {
			...
			if (currentCluster.length > 1) {
				...
				if (singleTooltip.yPosition < requiredMinimumY) singleTooltip.yPosition = requiredMinimumY;
		...
		if (lastTooltipBottom > actualClipRect.y + actualClipRect.height) {
			...
			for (var currentIndex = itemCount - 1; currentIndex >= 0; currentIndex--) {
				...
				var previousTooltipBottom = currentIndex === 0 ? actualClipRect.y : sortedTooltipItems[currentIndex - 1].yPosition + sortedTooltipItems[currentIndex - 1].height;
				var availableSpace = currentTooltipY - previousTooltipBottom;
				if (availableSpace > overflowAmount) {
					for (var shiftIndex = currentIndex; shiftIndex < itemCount; shiftIndex++) sortedTooltipItems[shiftIndex].yPosition -= overflowAmount;
					break;
				}
			}
		}
		...
		if (firstTooltip.yPosition < actualClipRect.y) {
			...
			for (var currentIndex = 0; currentIndex < itemCount; currentIndex++) {
				...
				var availableSpace = (currentIndex === itemCount - 1 ? actualClipRect.height : sortedTooltipItems[currentIndex + 1].yPosition) - currentTooltipBottom;
				if (availableSpace > overflowAmount) {
					for (var shiftIndex = currentIndex; shiftIndex >= 0; shiftIndex--) sortedTooltipItems[shiftIndex].yPosition += overflowAmount;
					break;
				}
			}
		}
	...
	*/
	Tooltip.prototype.handleSplitPosition = function(splitIndex, groupElement, areaBounds, arrowLocation, tipLocation) {
		...
		if (isConnectorLineNeeded) {
			...
			if (document.getElementById(this.element.id + "_tooltip_connector_" + splitIndex)) removeElement(this.element.id + "_tooltip_connector_" + splitIndex);
			...
			if (this.hasHorizontalOverflow) isLeft = true;
		}
		var rect = this.splitTooltipRectCollection[splitIndex];
		return {
			rect,
			tooltipOverflow: rect.y < 0 || rect.y + rect.height > areaBounds.height || !withInAreaBounds(pointLocationX, pointLocationY, this.splitClipBounds[splitIndex]),
			isLeft,
			isConnectorLineNeeded
		};
	};
	Tooltip.prototype.createTooltipElement = function() {
		this.textElements = [];
		if (!this.template || this.shared) {
			if (this.enableRTL) this.element.setAttribute("dir", "ltr");
			var svgObject = this.renderer.createSvg({ id: this.element.id + "_svg" });
			this.element.appendChild(svgObject);
			if (this.split) this.configureSplitTooltipElements(svgObject);
			else {
	...
	*/
	...
	Tooltip.prototype.drawMarker = function(isBottom, isRight, size, splitIndex) {
		...
		var x = (this.enableRTL ? this.split ? this.splitTooltipRectCollection[splitIndex].width - size - this.marginX : this.elementSize.width - size / 2 : this.marginX * 2 + size / 2) + (isRight ? this.arrowPadding : 0);
			...
			if (!(this.split && splitIndex !== index)) {
				if (shape !== "None") {
					...
					if (this.markerPoint[this.split ? 0 : count]) {
						var padding = 0;
						if (this.header.indexOf("<br") > -1) padding = this.header.split(/<br.*?>/g).length + (this.split ? splitIndex : count);
						...
						markerGroup.appendChild(drawSymbol(new TooltipLocation(x, this.markerPoint[this.split ? 0 : count] - this.padding + (isBottom ? this.arrowPadding : padding)), shape, new Size(size, size), this.markerImage, shapeOption, "img", tooltipContent));
					}
					count++;
				}
			}
	...
	Tooltip.prototype.renderTooltipElement = function(areaBounds, location, splitIndex) {
		...
		if (!isNullOrUndefined(groupElement)) {
			if (this.header !== "" && this.showHeaderLine) this.elementSize.height += this.marginY;
			if (this.split) {
				...
				if (!this.inverted) {
					...
					if (this.allowHighlight) rect.x += isLeft ? this.highlightPadding : -(2 * this.highlightPadding);
				}
			}
			if (this.header !== "" && this.showHeaderLine && !this.split) {
				...
				if (this.isWrap && typeof wrapHeader === "string" && (wrapHeader.indexOf("<") > -1 || wrapHeader.indexOf(">") > -1)) wrapPadding = wrapHeader.split("<br>").length;
				if (this.header.indexOf("<br") > -1) padding = 5 * (this.header.split(/<br.*?>/g).length - 1);
				var font = extend({}, this.textStyle, null, true)["properties"];
				...
				var direction = "M " + (this.marginX * 3 + (!isLeft && !isTop && !isBottom ? this.arrowPadding : 0)) + " " + headerSize + "L " + (rect.width + (!isLeft && !isTop && !isBottom ? this.arrowPadding : 0) - this.marginX * 2) + " " + headerSize;
				var pathElement_1 = this.renderer.drawPath({
					"id": this.element.id + "_header_path",
					"stroke-width": 1,
					"fill": null,
					"opacity": this.theme === "Material3" || this.theme === "Material3Dark" ? .2 : .8,
			...
			if (this.enableAnimation && !this.isFirst && !this.crosshair) this.animateTooltipDiv(this.split ? groupElement : tooltipDiv, rect);
			else this.updateDiv(this.split ? groupElement : tooltipDiv, rect.x, rect.y);
			...
			if (this.enableShadow && this.theme !== "Bootstrap4" || this.theme.indexOf("Fluent2") > -1) {
				...
				if (this.theme === "Tailwind" || this.theme === "TailwindDark" || this.theme === "Tailwind3" || this.theme === "Tailwind3Dark" || this.theme === "Bootstrap5" || this.theme === "Bootstrap5Dark") pathElement.setAttribute("box-shadow", "0px 1px 2px rgba(0, 0, 0, 0.06), 0px 1px 3px rgba(0, 0, 0, 0.1)");
				else pathElement.setAttribute("filter", Browser.isIE ? "" : "url(#" + shadowId + ")");
				var shadow = "<filter id=\"" + shadowId + "\" height=\"130%\"><feGaussianBlur in=\"SourceAlpha\" stdDeviation=\"3\"/>";
				if (this.theme.indexOf("Fluent2") > -1) shadow += "<feOffset dx=\"-1\" dy=\"3.6\" result=\"offsetblur\"/><feComponentTransfer><feFuncA type=\"linear\" slope=\"0.2\"/>";
				else shadow += "<feOffset dx=\"3\" dy=\"3\" result=\"offsetblur\"/><feComponentTransfer><feFuncA type=\"linear\" slope=\"0.5\"/>";
			...
			if (!isNullOrUndefined(this.border.dashArray)) pathElement.setAttribute("stroke-dasharray", this.border.dashArray);
	...
	Tooltip.prototype.changeText = function(point, isBottom, isRight, splitIndex) {
		...
		if (isBottom) element.setAttribute("transform", "translate(0," + this.arrowPadding + ")");
		if (isRight) element.setAttribute("transform", "translate(" + this.arrowPadding + " 0)");
	};
	Tooltip.prototype.findFormattedText = function(splitIndex) {
		this.formattedText = [];
		if (this.header.replace(/<b>/g, "").replace(/<\/b>/g, "").trim() !== "" && !this.split) this.formattedText = this.formattedText.concat(this.header);
	...
	Tooltip.prototype.renderText = function(isRender, splitIndex) {
		...
		this.isWrap = false;
		var anchor = document.body.getAttribute("dir") === "rtl" && !this.enableRTL ? "end" : "start";
		...
		if (this.controlName === "Chart" && parseFloat(fontSize) < parseFloat(font.size || this.themeStyle.textStyle.headerTextSize)) textHeight = parseFloat(font.size || this.themeStyle.textStyle.size) - parseFloat(fontSize);
		...
		for (var k = 0, pointsLength = this.formattedText.length; k < pointsLength; k++) {
			...
			if (textCollection[0] === "") continue;
			if (k !== 0 || headerContent === "") this.markerPoint.push((headerContent !== "" && this.showHeaderLine ? this.marginY : 0) + options.y + height - (textHeight !== 0 ? textHeight / this.markerSize * (parseFloat(font.size || this.themeStyle.textStyle.headerTextSize) / this.markerSize) : 0));
			for (var i = 0, len = textCollection.length; i < len; i++) {
				...
				if (this.enableRTL && lines.length > 0) {
					...
					if (colonCount > 0 && (this.controlName === "Sankey" ? colonCount === 1 : true)) {
				...
				for (var j = 0, len_1 = lines.length; j < len_1; j++) {
					line = lines[j];
					if (this.enableRTL && line !== "" && this.isRTLText(line)) line = line.concat("‎");
					if (!/\S/.test(line) && line !== "") line = " ";
					if (!isColumn && line === " " || line.replace(/<b>/g, "").replace(/<\/b>/g, "").trim() !== "") {
						subWidth += line !== " " ? spaceWidth : 0;
						if (isColumn && !isRow) {
							if (this.header.indexOf("<br") > -1 && k !== 0) headerSpace += this.header.split(/<br.*?>/g).length;
							tspanOption = {
							...
							headerSpace = null;
						} else if (isRow && isColumn) tspanOption = { x: headerContent === "" ? this.marginX * 2 + (markerSize + markerPadding) : this.marginX * 2 + (this.isWrap ? markerSize + markerPadding : 0) };
						else tspanOption = {};
						isColumn = false;
		...
		if (this.split) {
			...
			var rect = new Rect(this.splitLocations[splitIndex].x + this.splitClipBounds[splitIndex].x + padding, this.splitLocations[splitIndex].y + this.splitClipBounds[splitIndex].y - elementHeight / 2, elementWidth, elementHeight);
	...
	Tooltip.prototype.getTooltipTextContent = function(tooltipText) {
		...
		if (isNullOrUndefined(characterCollection)) return tooltipText;
		var isRtlText = this.isRTLText(tooltipText);
		for (var i = 0; i < characterCollection.length; i++) if (this.isValidHTMLElement(characterCollection[i].replace("<", "").replace("/", "").replace(">", "").trim())) tooltipText = tooltipText.replace(characterCollection[i], isRtlText ? "‎" : "");
		return tooltipText;
	};
	...
	Tooltip.prototype.createTemplate = function(areaBounds, location, splitIndex) {
		...
		if (this.split && !isNullOrUndefined(splitIndex)) {
			...
			if (splitParentElem) remove(splitParentElem);
		} else {
			var firstElement = !isNullOrUndefined(parent) ? parent.firstElementChild : null;
			if (firstElement) remove(firstElement);
		}
		if (!argsData.cancel) {
			var elem = createElement("div", { id: this.split ? this.element.id + "parent_template" + splitIndex : this.element.id + "parent_template" });
			var templateElement = void 0;
			if (this.controlName === "Chart" && this.shared) for (var i = 0; i < this.data.length; i++) {
				var sharedTemplateElement = this.templateFn(this.data[i], this.controlInstance, elem.id, elem.id + "_blazorTemplate", "");
				if (i === 0) templateElement = sharedTemplateElement;
				else if (sharedTemplateElement.length > 1) templateElement[i].outerHTML = sharedTemplateElement[i].outerHTML || sharedTemplateElement[i].textContent;
				else templateElement[templateElement.length - 1].outerHTML += sharedTemplateElement[0].outerHTML;
			}
			else templateElement = this.templateFn(this.split ? this.data[splitIndex] : this.data, this.controlInstance, elem.id, elem.id + "_blazorTemplate", "");
			while (templateElement && templateElement.length > 0) if (isBlazor() || templateElement.length === 1) {
				elem.appendChild(templateElement[0]);
				templateElement = null;
			} else elem.appendChild(templateElement[0]);
			if (!isNullOrUndefined(parent)) parent.appendChild(elem);
			...
			if (this.split && !isNullOrUndefined(splitIndex)) {
				...
				var elementWidth = this.elementSize.width + 2 * this.marginX;
				var elementHeight = this.elementSize.height + 2 * this.marginY;
				tooltipRect = new Rect(this.splitLocations[splitIndex].x + this.splitClipBounds[splitIndex].x + padding, this.splitLocations[splitIndex].y + this.splitClipBounds[splitIndex].y - elementHeight / 2, elementWidth, elementHeight);
			} else tooltipRect = this.shared ? this.sharedTooltipLocation(areaBounds, this.location.x, this.location.y) : this.tooltipLocation(areaBounds, location, new TooltipLocation(0, 0), new TooltipLocation(0, 0));
			if (this.enableAnimation && !this.isFirst && !this.crosshair) this.animateTooltipDiv(this.element, tooltipRect);
			else this.updateDiv(element, tooltipRect.x, tooltipRect.y);
		...
		} else remove(getElement(this.element.id + "_tooltip"));
	};
	Tooltip.prototype.sharedTooltipLocation = function(bounds, x, y) {
		var width = this.elementSize.width + 2 * this.marginX;
		var height = this.elementSize.height + 2 * this.marginY;
		var pointerTooltipRect = new Rect(this.shared || this.inverted ? x + 2 * this.padding : x - width / 2, this.shared || this.inverted ? y - height / 2 : y - (height + this.padding), width, height);
		var tooltipRect = this.followPointer ? pointerTooltipRect : new Rect(x + 2 * this.padding, y - height - this.padding, width, height);
		if (tooltipRect.y < bounds.y) tooltipRect.y += tooltipRect.height + 2 * this.padding;
		if (tooltipRect.y + tooltipRect.height > bounds.y + bounds.height) tooltipRect.y = Math.max(bounds.y + bounds.height - (tooltipRect.height + 2 * this.padding), bounds.y);
		if (tooltipRect.x + tooltipRect.width > bounds.x + bounds.width) tooltipRect.x = bounds.x + this.location.x - (tooltipRect.width + 4 * this.padding);
		if (tooltipRect.x < bounds.x) tooltipRect.x = bounds.x;
		return tooltipRect;
	};
	/** @private */
	Tooltip.prototype.getCurrentPosition = function(bounds, symbolLocation, arrowLocation, tipLocation) {
		...
		if (position === "Top" || position === "Bottom") {
			...
			if (position === "Bottom") location.y = symbolLocation.y + clipY + markerHeight;
			...
			if (position === "Left") location.x = symbolLocation.x + clipX - markerHeight - (width + this.arrowPadding);
	...
	Tooltip.prototype.tooltipLocation = function(bounds, symbolLocation, arrowLocation, tipLocation) {
		if (!isNullOrUndefined(this.tooltipPlacement)) return this.getCurrentPosition(bounds, symbolLocation, arrowLocation, tipLocation);
		var location = new TooltipLocation(symbolLocation.x, symbolLocation.y);
		var width = this.elementSize.width + 2 * this.marginX;
		var height = this.elementSize.height + 2 * this.marginY;
		var markerHeight = this.offset;
		...
		if (!this.inverted) {
			...
			arrowLocation.x = tipLocation.x = width / 2;
			if ((location.y < boundsY || this.isNegative) && !(this.controlName === "Progressbar")) location.y = (symbolLocation.y < 0 ? 0 : symbolLocation.y) + clipY + markerHeight;
			if (location.y + height + this.arrowPadding > boundsY + bounds.height) location.y = Math.min(symbolLocation.y, boundsY + bounds.height) + clipY - this.elementSize.height - 2 * this.padding - this.arrowPadding - markerHeight;
			if ((location.x + width > boundsX + bounds.width && location.y < boundsY || this.isNegative) && !(this.controlName === "Progressbar")) location.y = (symbolLocation.y < 0 ? 0 : symbolLocation.y) + clipY + markerHeight;
			...
			if (arrowLocation.x - this.arrowPadding < this.rx) arrowLocation.x = tipLocation.x = this.rx + this.arrowPadding;
			if (this.controlName === "Chart") {
				if (bounds.x + bounds.width - (location.x + arrowLocation.x) < this.areaMargin + this.arrowPadding || location.x + arrowLocation.x < this.areaMargin + this.arrowPadding) this.outOfBounds = true;
				if (this.template && location.y < 0) location.y = symbolLocation.y + clipY + markerHeight;
				if (!withInAreaBounds(location.x, location.y, bounds) || this.outOfBounds) {
					...
					if (location.x + this.arrowPadding + width > boundsX + bounds.width || this.isNegative) location.x = (symbolLocation.x > boundsX + bounds.width ? bounds.width : symbolLocation.x) + clipX - markerHeight - (this.arrowPadding + width);
					if (location.x < boundsX) location.x = (symbolLocation.x < 0 ? 0 : symbolLocation.x) + markerHeight + clipX;
			...
			location = new TooltipLocation(location.x + clipX + markerHeight, location.y + clipY - this.elementSize.height / 2 - this.padding);
			arrowLocation.y = tipLocation.y = height / 2;
			if (location.x + width + this.arrowPadding > boundsX + bounds.width || this.isNegative) location.x = (symbolLocation.x > bounds.width + bounds.x ? bounds.width : symbolLocation.x) + clipX - markerHeight - (width + this.arrowPadding);
			if (symbolLocation.x > clipWidth) location.x = clipWidth + clipX - (width + this.arrowPadding);
			if (location.x < boundsX) location.x = (symbolLocation.x < 0 ? 0 : symbolLocation.x) + clipX + markerHeight;
			if (location.x + width + this.arrowPadding > boundsX + bounds.width) location.x = (symbolLocation.x > bounds.width + bounds.x ? bounds.width : symbolLocation.x) + clipX - markerHeight - (width + this.arrowPadding);
			if (location.y <= boundsY) {
				arrowLocation.y -= boundsY - location.y;
				tipLocation.y -= boundsY - location.y;
				location.y = boundsY;
			...
			if (arrowLocation.y - this.arrowPadding < this.ry) arrowLocation.y = tipLocation.y = this.ry + this.arrowPadding;
			if (this.controlName === "Chart") {
				if (location.y + arrowLocation.y < this.areaMargin + this.arrowPadding || bounds.y + bounds.height - (location.y + arrowLocation.y) < this.areaMargin + this.arrowPadding) this.outOfBounds = true;
				if (!withInAreaBounds(location.x, location.y, bounds) || this.outOfBounds) {
					this.inverted = !this.inverted;
					...
					if (location.y < boundsY || this.isNegative) location.y = (symbolLocation.y < 0 ? 0 : symbolLocation.y) + markerHeight + clipY;
					if (location.y + this.arrowPadding + height > boundsY + bounds.height) location.y = Math.min(symbolLocation.y, boundsY + bounds.height) + clipY - this.elementSize.height - 2 * this.padding - markerHeight - this.arrowPadding;
					tipLocation.x = width / 2;
	...
	Tooltip.prototype.animateTooltipDiv = function(tooltipDiv, rect) {
		...
		if (this.controlName === "Chart" && (this.shared || this.split) && !this.enableRTL) {
			...
			var width = this.elementSize.width + 2 * this.marginX;
			var targetRight = (this.availableSize && this.availableSize.width ? this.availableSize.width : this.areaBounds.x + this.areaBounds.width) - (rect.x + width);
	...
	Tooltip.prototype.updateDiv = function(tooltipDiv, x, y) {
		if (this.controlName === "Chart" && (this.shared && !this.crosshair || this.split) && (!this.enableRTL || this.split)) {
			...
			var width = this.elementSize.width + 2 * this.marginX;
			var right = (this.availableSize && this.availableSize.width ? this.availableSize.width : this.areaBounds.x + this.areaBounds.width) - (x + width);
	...
	Tooltip.prototype.updateTemplateFn = function() {
		if (this.template) try {
			if (typeof this.template !== "function" && document.querySelectorAll(this.template).length) this.templateFn = compile(document.querySelector(this.template).innerHTML.trim());
			else this.templateFn = compile(this.template);
		} catch (e) {
	...
	Tooltip.prototype.fadeOut = function() {
		...
		if (tooltipElement) {
			...
			if (tooltipGroup_1 && tooltipGroup_1.nodeType !== Node.ELEMENT_NODE) tooltipGroup_1 = tooltipElement.firstElementChild;
			if (this.isCanvas && !this.template) tooltipGroup_1 = document.getElementById(this.element.id + "_group") ? document.getElementById(this.element.id + "_group") : tooltipGroup_1;
			if (!tooltipGroup_1) return null;
	...
	Tooltip.prototype.endAnimation = function(tooltipGroup) {
		...
		if (this.template) tooltipGroup.style.display = "none";
	...
	*/
	...
	__decorate([Property(false)], Tooltip.prototype, "split", void 0);
	__decorate([Property(false)], Tooltip.prototype, "followPointer", void 0);
	...
	__decorate([Property(.75)], Tooltip.prototype, "opacity", void 0);
	__decorate([Complex({
		size: "12px",
		fontWeight: null,
		color: null,
	...
	__decorate([Property([])], Tooltip.prototype, "splitClipBounds", void 0);
	__decorate([Property([])], Tooltip.prototype, "palette", void 0);
	__decorate([Property([])], Tooltip.prototype, "shapes", void 0);
	__decorate([Property("")], Tooltip.prototype, "markerImage", void 0);
	__decorate([Complex({
		x: 0,
		y: 0
	...
	__decorate([Complex({
		x: 0,
		y: 0
	}, ToolLocation)], Tooltip.prototype, "splitLocations", void 0);
	__decorate([Property([])], Tooltip.prototype, "seriesTypes", void 0);
...
//# sourceMappingURL=@syncfusion_ej2-svg-base.js.map