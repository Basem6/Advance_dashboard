...
let TreeView = TreeView_1 = class TreeView extends Component {
        ...
        if (this.isInitalExpand && eUids.length > 0) {
            ...
            if (this.fields.dataSource instanceof DataManager) {
                ...
                for (let i = 0; i < eUids.length; i++) {
                    const uid = this.escapeHashInUid(eUids[parseInt(i.toString(), 10)]);
                    const eNode = select('[data-uid="' + uid + '"]', this.element);
    ...
    expandCallback(eUid, callback) {
        const uid = this.escapeHashInUid(eUid);
        const eNode = select('[data-uid="' + uid + '"]', this.element);
        if (!isNullOrUndefined(eNode)) {
            const icon = select('.' + EXPANDABLE, select('.' + TEXTWRAP, eNode));
            if (!isNullOrUndefined(icon)) {
    ...
    doSelectionAction() {
        ...
        if (sUids.length > 0) {
            ...
            for (let i = 0; i < sUids.length; i++) {
                const uid = this.escapeHashInUid(sUids[parseInt(i.toString(), 10)]);
                const sNode = select('[data-uid="' + uid + '"]', this.element);
